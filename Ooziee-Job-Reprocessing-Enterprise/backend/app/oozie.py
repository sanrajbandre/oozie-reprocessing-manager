...
class OozieClient:
    ...
    def job_info(self, job_id: str) -> Dict[str, Any]:
        ...
        r = requests.get(url, params={"show": "info"}, timeout=self.timeout)
        r.raise_for_status()
        return r.json()
    ...
    def rerun(self, job_id: str, conf: Optional[Dict[str, str]] = None, params: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        ...
        body = ""
        if conf:
            props = "".join([f"<property><name>{k}</name><value>{v}</value></property>" for k, v in conf.items()])
            body = f"<configuration>{props}</configuration>"
        r = requests.put(url, params=q, data=body.encode("utf-8"), headers=headers, timeout=self.timeout)
        r.raise_for_status()
        try:
            return r.json()
        except Exception:
